...
const filterDuplicates = (arr: SunoMedia[]) => {
  const seen = new Set()
  return arr.filter((item) => {
    const createTime = item.created_at?.slice(0, 19)
    if (!seen.has(createTime)) {
      seen.add(createTime)
      return true
    }
    return false
...
const initLoad = () => {
  ...
  list.value = filterDuplicates(arr.reverse())
}
    ...
    <div v-for="item in list" :class="getNowCls(item)" class="flex relative  justify-between items-start p-2 hover:dark:bg-black hover:bg-gray-200 border-b-[1px] border-gray-500/10 ">
      ...
      <div class="flex-1  pl-2">
        ...
        <div class="text-right text-[14px] flex justify-end items-center space-x-2  ">
          <div v-if="item.metadata && item.metadata.duration" class="text-[10px] flex items-center border-[1px] border-gray-500/30 px-0.5 list-none rounded-md">
