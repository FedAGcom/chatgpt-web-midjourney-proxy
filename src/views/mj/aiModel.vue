...
import { getModelLabel } from '@/utils/functions/getModelLabel'
import unavaliableModal from '@/views/mj/unavaliableModal.vue'
...
const modellist = computed(() => { //
  ...
  if (homeStore.myData.session.cmodels) {
    ...
    addModel.map((o) => {
      const newLabel = getModelLabel(o)
      const newModel = { label: newLabel, value: o }
      rz.push(newModel)
    })
...
//   const selected = modellist.value.find(model => model.value === value)
//   if (selected && !selected.avaliable) {
//     // selectedModel.value = selected;
//     isModalVisible.value = true
//   }
//   else {
//     nGptStore.value.model = value
//   }
// }
...
const saveChat = (type: string) => {
  if (chatStore.avaliableModels.includes(nGptStore.value.model)) {
...
watch(() => userStore.userInfo.role, (role) => {
  chatStore.setAvailiableModels(role)
})
...
<template>
  ...
  <unavaliableModal v-if="show" v-model:visible="show" />
</template>
