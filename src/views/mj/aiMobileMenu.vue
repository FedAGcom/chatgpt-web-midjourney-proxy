...
<template>
  <div class=" bg-gray-100 dark:bg-[#282832] h-[55px] flex  justify-around  items-center dark:text-white/70 ">
    ...
    <div v-if="!isDisableMenu ('draws')" class="flex items-center justify-center flex-col " @click="homeStore.setMyData({ act: 'help' }) ">
      <SvgIcon icon="ic:outline-palette" class="text-3xl" />
      <div class="text-[13px]">
        {{ $t('helptab.help') }}
      </div>
    </div>
    <div v-if="!isDisableMenu ('gallery')" class="flex items-center justify-center flex-col " @click="showTariff = true ">
      <SvgIcon icon="material-symbols:imagesmode-outline" class="text-3xl" />
      <div class="text-[13px]">
        {{ $t('tarifftab.tariff') }}
      </div>
